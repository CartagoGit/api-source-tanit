...
export async function ensureInsideAny(
  ...
  return {
    ok: false,
    resolved: realTarget,
    // The CLI output surface speaks English (README, `--help`, the
    // published package — lint:output-language enforces it). The
    // Spanish wording lives one layer up, in `generate`'s containment
    // message that embeds this reason.
