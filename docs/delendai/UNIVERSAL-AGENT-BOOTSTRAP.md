# Universal agent bootstrap — vendored from `@delendai/core`

> **Provenance:** snapshot of
> `delendai/docs/delendai/AGENT-BOOTSTRAP.md` (upstream package
> `@delendai/core`), vendored into this repository so consumers do
> **not** need a sibling checkout of `delendai`.
>
...
>
> **Refresh policy:** when upstream delendai ships a meaningful
...
- Core stays agnostic. No project vocabulary inside `@delendai/core`.
...
- **Agents and tools invoke shell through `bash`, never `zsh` or
  ...
  `@delendai/core/public` when available).
...
Do **not** look for a sibling `delendai` checkout or an external
`REPO-RULES.md`.
...
When the `@delendai/status-marker` plugin is loaded (`delendai_overview`
...
launch delendai via the **published** CLI when available
(`bunx --package @delendai/cli mcpv __serve ...`). A local host script
