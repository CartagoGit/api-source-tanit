...
**delendai orchestrator** can drive work in this repo through the
...
orchestrator falls back to raw `delendai_overview` and human
routing.
...
- A Copilot-mode agent that does its own commits. All git work
  flows through the delendai `git` plugin tool, not bespoke shell.
...
- **Gate**: orchestrator can pick this agent via `delendai_agent_catalog`.

- The agent uses `delendai_analyze_project` as the entry point,
  then `postman_exporter_summary` to gather DOM data, and returns
  the discriminated output above.
- **Acceptance**:
  - `delendai_agent_catalog` lists the agent.
...
All 4 agents are listed in `delendai_agent_catalog`. Running the
