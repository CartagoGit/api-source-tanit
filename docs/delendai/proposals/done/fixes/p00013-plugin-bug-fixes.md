...
> `path` de `delendai.config.json` resuelve. La parte de
...
Make `bun /home/cartago/_packages/delendai/tools/scripts/host/host-server.script.ts
--workspace . --config ./delendai.config.json` boot cleanly with the
...
```
$ bun /home/cartago/_packages/delendai/tools/scripts/host/host-server.script.ts \
    --workspace . --config ./delendai.config.json
[delendai] boot failed: ReferenceError: NAMESPACE is not defined
    at register (.../postman-exporter/src/lib/tools/generate.tool.ts:47:10)
    at createMcpProject (.../delendai/packages/core/src/lib/project/create-mcp-project.ts:310:22)
    ...
```
...
- `bun /home/cartago/_packages/delendai/tools/scripts/host/host-server.script.ts --workspace . --config ./delendai.config.json`
  no longer crashes with `ReferenceError`. Process is killed by
  SIGTERM after the 5s smoke-test window; exit code is 143.
- `delendai_overview { compact: true }` lists the 4 plugin tools
...
2. Call `delendai_overview { compact: true }` and assert that
   the 4 `postman_exporter_*` tools are listed under the
   `postman-exporter` and `postman-exporter-testing` plugins.
3. Call `delendai_proposals_proposal_board` and assert the
   12 proposals from `ready/` are listed.
4. Call `delendai_proposals_proposals_auto_work` once and
...
- Repackaging `delendai` to npm (covered by p00007 / p00008).
- Switching the plugin loader from `delendai.config.json` to
...
- Moving plugins to `plugins/delendai/{}` (decided out: the
