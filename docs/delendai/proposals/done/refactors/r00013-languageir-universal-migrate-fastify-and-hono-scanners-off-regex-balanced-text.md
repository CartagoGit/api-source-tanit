...
dependsOn:
  ...
  - r00014
shippedIn:
  - a6f40f6  # S1 extractFastifyRoutesFromProgram + S2 extractHonoRoutesFromProgram + extractRoutes dispatcher
inProgressReason: "S3 (scanner rewiring) and S4 (multi-router fixtures + e2e tests) not implemented. fastify.scanner.ts and hono.scanner.ts still use findOutsideStrings + findAllBalanced (regex/balanced-text). Follow-up tracked in r00018."
---
