...
La auditoría completa (brief canónico de `audit_plan`, scope full, mode general) dejó 23 hallazgos: 3 FATAL, 7 BAD, 8 MINOR y 5 OK/GOOD. El documento vive en `docs/delendai/audits/08-08-2026- Claude Code (claude-opus-5).md` y `audit_consolidate` lo parsea entero. Los tres FATAL no son bugs de comportamiento: son sitios donde el proyecto dice una cosa y hace otra — cuatro tools sin `outputSchema` que el propio bootstrap exige, ninguna escritura durable atómica, y una propiedad colada fuera del contrato con `as any`. Por debajo hay una causa común: el bootstrap del proyecto describe una arquitectura que se sustituyó hace tres reorganizaciones, y por eso las reglas que enuncia no las comprueba nadie.
...
- Desbloquear p00007, que depende de que @delendai/core se publique en npm

---

> **Date**: 2026-08-08 · **Reviewer**: Claude Code (claude-opus-5) · **Scope**: Full audit · **Mode**: general.
> **Brief**: `delendai_audit_audit_plan { scope: "full", mode: "general" }`.
...
**Fichero**: `packages/plugins/delendai_expostman/src/lib/tools/generate.tool.ts#L55`
...
**Impact**: un agente que llama a `delendai_expostman_generate` recibe
...
**Fichero**: `docs/delendai/AGENT-BOOTSTRAP.md#L225`
...
(§3.1 y §3.5, es `packages/plugins/delendai_expostman/`),
...
registra `` `${ctx.namespacePrefix}_${TOOL_ID}` `` → `delendai_expostman_generate`.
...
vienen de un agente
(`packages/plugins/delendai_expostman/src/lib/contracts/cli-path.constant.ts`),
...
**Fichero**: `packages/plugins/delendai_expostman/src/lib/helpers/runner.helper.ts#L245`
...
**Fichero**: `packages/plugins/delendai_expostman/src/lib/contracts/namespace.ts#L2`
...
**Fichero**: `delendai.config.json` → `plugins.search.options.roots`
...
**Fichero**: `docs/delendai/AGENT-BOOTSTRAP.md#L155`
...
**Fichero**: `packages/plugins/delendai_expostman/src/lib/tools/generate.tool.ts`
...
- 🔴 **P0** — Declarar `outputSchema` en los 4 tools y añadir el gate que lo exija · `packages/plugins/delendai_expostman/src/lib/tools/*.tool.ts`
...
- 🟠 **P1** — Reescribir §3.1, §3.5 y §3.8 del bootstrap contra el código actual · `docs/delendai/AGENT-BOOTSTRAP.md`
...
- **Status**: done
- **Files**: `docs/delendai/proposals/done/audits/a00001-auditor-a-completa-2026-08-08-cerrar-los-23-hallazgos.md`
- **Gate**: none
- acceptance:
