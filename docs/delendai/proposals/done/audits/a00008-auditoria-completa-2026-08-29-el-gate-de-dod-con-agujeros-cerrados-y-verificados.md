...
- TARGET_EVIDENCE: package.json (name, bin, exports), bun lockfile, tsconfigs por
  ...
  de propuestas en docs/delendai/proposals/
```
...
- `delendai_security_security_audit` (via plugin del host) → sin hallazgos en
...
- **Comportamiento**: `.mcp.json` y `.vscode/mcp.json` arrancan el host con
  `../delendai/tools/scripts/host/host-server.script.ts`, un checkout que
  ...
  pre-publish y cambiando a `@delendai/cli` cuando exista).
...
en `delendai.config.json` (placeholders validos de runtime, no rutas rotas);
...
2. **Dependencia pre-publish del checkout hermano** (F-005): hasta que exista
   `@delendai/cli` publicado, el arranque MCP depende de un arbol externo;
...
- **No** desbloquear F-005 a mano: sin `@delendai/cli` publicado, quitar el
...
(`delendai`) declarado una vez en `.mcp.json` y derivado por `mcp:sync`.
