...
date: 2026-07-31
superseded_by: docs/delendai/AGENT-BOOTSTRAP.md
...
> `docs/delendai/AGENT-BOOTSTRAP.md` (§3.5 file naming, §3.6 plugin
...
2. Add a new tool to the delendai plugin (per `plugins/postman-exporter/`).
...
- Sections:
  1. **Adding a router adapter** (`IRouterAdapter` shape, where to
     ...
     "Next.js" adapter).
  2. **Adding a tool to the delendai plugin** (the `tool.ts` shape,
