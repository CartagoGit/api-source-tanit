...
- **Flask sin bodies reales**: 0 de 14 endpoints resuelven reglas. El
  provider solo reconoce `flask_pydantic` y no inspecciona modelos
  inline. Declarado en `capabilities.validation: false` del contrato de
  test, no oculto.
- **ASP.NET minimal APIs**: `app.MapGet("/users", …)` en `Program.cs` no
  se detecta; solo controladores.
