...
| Slice | Commit | Notas |
|---|---|---|
| **S0** CI reproducible | `9e00a5e` | Materializado checkout de `CartagoGit/mcp-vertex` en `validate.yml` fijado a SHA. Constante `MCP_VERTEX_SHA` (`packages/contracts/constants/core/mcp-vertex-sha.constant.ts`). Aserción nueva en `validate-package`: el plugin NO entra en el tarball (`files` con negación explícita en `package.json` raíz). |
...
- [x] `docs/mcp-vertex/proposals/INDEX.md` actualizado.
