...
describe("snapshot activation CAS", () => {
  it("allows one writer for a revision and rejects the stale writer", () => {
    ...
    const snapshots = new SqliteSnapshotRepository(db); const transactions = new SnapshotTransactionService(db, snapshots); const project = new SqliteProjectRepository(db); const activation = new SnapshotActivationService(project, () => "2026-09-08T00:00:00.000Z");
  ...
  it("rejects a missing snapshot without changing the project", () => {
    const db = new Database(":memory:"); migrateStateDatabase(db); db.exec("INSERT INTO projects (project_id, root_path, created_at, updated_at) VALUES ('p', '/tmp/p', 'now', 'now')");
    const project = new SqliteProjectRepository(db);
  ...
  it("rejects a complete snapshot from another project", () => {
    ...
    const snapshots = new SqliteSnapshotRepository(db); const transactions = new SnapshotTransactionService(db, snapshots); const project = new SqliteProjectRepository(db);
  ...
  it("rejects a building snapshot", () => {
    const db = new Database(":memory:"); migrateStateDatabase(db); db.exec("INSERT INTO projects (project_id, root_path, created_at, updated_at) VALUES ('p', '/tmp/p', 'now', 'now')");
    const snapshots = new SqliteSnapshotRepository(db); const project = new SqliteProjectRepository(db);
  ...
  it("rejects a failed snapshot", () => {
    ...
    expect(project.activate(snapshot.projectId, { ...snapshot, status: "failed", failure: "failed" }, 0, "later")).toBe(false);
    expect(project.read(snapshot.projectId)?.active_snapshot_id).toBe(null); expect(project.read(snapshot.projectId)?.revision).toBe(0); db.close();
  });
});
