...
describe("snapshot crash recovery", () => {
  it("does not expose a building snapshot as active state", () => {
    ...
    const repository = new SqliteSnapshotRepository(db); const service = new SnapshotTransactionService(db, repository); const id = { kind: "snapshot" as const, value: "building-1" };
