...
type BunDatabase = ConstructorParameters<typeof Database>[0] extends never ? never : Database;
...
const database = () => { const db = new Database(":memory:") as BunDatabase; migrateStateDatabase(db as never); db.exec("INSERT INTO projects (project_id, root_path, created_at, updated_at) VALUES ('project-demo', '/tmp/demo', 'now', 'now')"); return db; };

describe("sqlite snapshot repository", () => {
  it("writes atomically, reads after restart, and does not persist secrets", () => {
    const db = database(); const repository = new SqliteSnapshotRepository(db as never); const service = new SnapshotTransactionService(db as never, repository);
    const complete = service.write(snapshot("1") as Parameters<typeof service.write>[0]);
    ...
    const restarted = new SqliteSnapshotRepository(db as never); expect(restarted.get(complete.snapshotId)?.services[0]?.operations[0]?.path).toBe("/users");
    db.close();
  });

  it("keeps complete snapshots immutable and rolls back failed writes", () => {
    const db = database(); const repository = new SqliteSnapshotRepository(db as never); const service = new SnapshotTransactionService(db as never, repository); const complete = service.write(snapshot("1") as Parameters<typeof service.write>[0]);
    const projects = new SqliteProjectRepository(db as never);
    ...
    expect(() => service.write(failed as Parameters<typeof service.write>[0], () => { throw new Error("scan failed"); })).toThrow("scan failed");
