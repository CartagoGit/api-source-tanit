...
import {
  ...
  toPostmanUri,
} from "../../service/adapters/parsed-route-to-spec.adapter";
import type {
  IProjectMatch,
  IRouteScanner,
  IValidationSpec,
...
describe("buildSpecsFromScanner — valores de ejemplo", () => {
  test("un email usa un ejemplo con formato de email", async () => {
    ...
    expect(String(result.specs[0]?.body?.["email"])).toContain("@");
  });
  ...
  test("un enum usa su primer valor", async () => {
    ...
    expect(result.specs[0]?.body?.["role"]).toBe("admin");
  });
  ...
  test("un booleano usa true", async () => {
    ...
    expect(result.specs[0]?.body?.["activo"]).toBe(true);
  });
